...
<head>
  ...
  <style>
    :root {
      ...
      --blue:    #4488ff;
      --purple:  #9944ff;
      --repo-glow: 139,124,255;  /* Repository の光（親＝Core と同一）。本が継承する紫 */
      --green:   #00ff88;
      --red:     #ff4466;
      --yellow:  #ffcc00;
      --text:    #cce8ff;
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .nav { display: flex; gap: 20px; align-items: center; position: relative; top: 6px; }
    .nav a { font-size: 0.73rem; color: var(--text-dim); letter-spacing: 0.12em; text-transform: uppercase; transition: color 0.2s; }
    .nav a:hover { color: var(--cyan); }
    /* 建物の案内板：今いるフロアに ● が点く */
    .nav a.nav-active { color: #e8edf5; }
    .nav a.nav-active::before { content: '●'; color: var(--cyan); font-size: 0.55em; margin-right: 7px; vertical-align: 2px; }
    /* 📘 Repository Intelligence（本館）への扉：本を開く行為。ラベルなし・ホバーで名前 */
    /* ② status pill から 16px 逃がして"一冊だけ置いてある"呼吸を作る */
    .repo-door { position: relative; display: flex; align-items: center; margin: 0 22px 0 4px; text-decoration: none;
      filter: drop-shadow(0 3px 9px rgba(0,0,0,.40)); }
    /* ① 💜 Repository が応える：ホバー即、紫の脈（Coreの鼓動を継承）がパッと広がる＝ドクン */
    .repo-pulse{ position:absolute; top:50%; left:50%; width:44px; height:44px; border-radius:50%;
      transform:translate(-50%,-50%) scale(.5); opacity:0; z-index:0; pointer-events:none;
      box-shadow:0 0 0 2px rgba(var(--repo-glow),.85),0 0 20px 5px rgba(var(--repo-glow),.5); }
    .repo-door:hover .repo-pulse{ animation:cl-beat .5s cubic-bezier(.2,.7,.3,1); }
    @keyframes cl-beat{ 0%{opacity:0;transform:translate(-50%,-50%) scale(.5)} 20%{opacity:1} 100%{opacity:0;transform:translate(-50%,-50%) scale(1.75)} }
    /* ② 📘 本は Core の紫を継承して光る（ドクンの直後 0.06s に パッ）。シアンでなく紫＝生命 */
    .repo-door img { position:relative; z-index:1; width: 50px; height: auto; display: block; transition: transform .16s ease, filter .18s ease; }
    .repo-door:hover img { transform: translateY(-1px);
      filter: drop-shadow(0 0 4px rgba(var(--repo-glow),.9)) drop-shadow(0 0 12px rgba(var(--repo-glow),.5));
      transition: transform .16s ease, filter .2s ease .06s; }
    /* ③ ⚡ データが流れる：紫の後（0.15s遅れ）シアンのパケットが左右から本へ流れ込む＝アクセス */
    .repo-door::before, .repo-door::after{ content:''; position:absolute; top:50%; z-index:0; pointer-events:none; opacity:0;
      left:50%; width:66px; height:2px; filter:drop-shadow(0 0 5px rgba(0,225,255,.85)); }
    .repo-door::before{ transform:translate(-118px,-50%);
      background:linear-gradient(90deg, rgba(0,225,255,0) 0%, rgba(0,225,255,.12) 55%, rgba(0,225,255,1) 100%); }
    .repo-door::after { transform:translate(52px,-50%);
      background:linear-gradient(90deg, rgba(0,225,255,1) 0%, rgba(0,225,255,.12) 45%, rgba(0,225,255,0) 100%); }
    .repo-door:hover::before{ animation: cl-pkt-l .5s cubic-bezier(.25,.7,.2,1) .15s; }
    .repo-door:hover::after { animation: cl-pkt-r .5s cubic-bezier(.25,.7,.2,1) .15s; }
    @keyframes cl-pkt-l{ 0%{opacity:0; transform:translate(-118px,-50%)} 26%{opacity:1} 100%{opacity:0; transform:translate(-44px,-50%)} }
    @keyframes cl-pkt-r{ 0%{opacity:0; transform:translate(52px,-50%)} 26%{opacity:1} 100%{opacity:0; transform:translate(-22px,-50%)} }
    @media (prefers-reduced-motion: reduce){ .repo-door:hover .repo-pulse, .repo-door:hover::before, .repo-door:hover::after{ animation:none } }
    /* ⑤ tip を本に近づけてラベル化（真下 9→3px）*/
    .repo-door-tip { position: absolute; top: calc(100% + 3px); right: 0; white-space: nowrap;
      font-family: 'JetBrains Mono', monospace; font-size: 10px; letter-spacing: 1.5px; text-transform: uppercase; color: #9db4dc;
      background: rgba(10,14,24,.92); border: 1px solid rgba(255,255,255,.1); padding: 6px 11px; border-radius: 8px;
      opacity: 0; transform: translateY(-4px); pointer-events: none; transition: opacity .2s ease, transform .2s ease; z-index: 30; overflow: hidden; }
    .repo-door:hover .repo-door-tip { opacity: 1; transform: translateY(0); }
    /* tip の下辺を、シアンが左→右へ一度だけ流れる（接続された合図）*/
    .repo-door-tip::after{ content:''; position:absolute; left:0; bottom:0; width:100%; height:1.5px;
      background:linear-gradient(90deg, transparent, rgba(0,225,255,.95), transparent); transform:translateX(-110%); }
    .repo-door:hover .repo-door-tip::after{ animation: cl-tip-sweep .55s ease .08s; }
    @keyframes cl-tip-sweep{ 0%{transform:translateX(-110%)} 100%{transform:translateX(110%)} }
    /* Status pill */
    .status-pill {
